...
export default function AddTaskScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#000000', padding: 12 }}>
      <Pressable
        onPress={() => router.replace(`/projectWindow?project=${projectID}`)}
      ...
      <Box style={{ marginBottom: 10, flex: 1 }}>
              ...
              tasks
                .filter(
                  (task) =>
                    ...
                    !task.parentTasks &&
                    (!task.childTasks || task.childTasks.length === 0) &&
                    task.title.toLowerCase().includes(searchInput.toLowerCase()) // Add search filter
                )
                .map((task) => {
