        ...
        <Box style={{ gap: 20 }}>
          <Box style={{ flexDirection: "row", justifyContent: "space-between" }}>
            ...
            <Text style={{ color: "white", fontSize: 16, fontWeight: 500 }}>
              {props.profiles.filter((t) => t.role === Role.PROJECT_MANAGER).length} employee/s
            </Text>
          </Box>
          {props.profiles.filter((t) => t.role === Role.PROJECT_MANAGER).length > 0 ? (
            <Grid _extra={{ className: "grid-cols-3 gap-4" }}>
              {props.profiles.reduce((acc: React.ReactNode[], t) => {
                if (t.role === Role.PROJECT_MANAGER) {
                  acc.push(
