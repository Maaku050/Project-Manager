...
export default function TaskCommentSection({ taskID }: TaskCommentSectionType) {
  ...
  return (
    ...
    >
      ...
      >
        <TextInput
          multiline // Add this to enable multiline
          value={value}
          onChangeText={(text) => {
            setValue(text)
            if (text.length === 0) {
          ...
          onKeyPress={({ nativeEvent }) => {
            if (nativeEvent.key === 'Enter') {
              if (nativeEvent.shiftKey) {
                ...
                // Enter only: Submit and prevent new line
                nativeEvent.preventDefault?.() // Prevent new line
                handleSubmitComment() // Your submit function
              }
            }
          }}
