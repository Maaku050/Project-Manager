      ...
      <Card style={{ backgroundColor: "#171717" }}>
        <Box style={{ gap: 20 }}>
          <Box style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={{ color: "white", fontSize: 20, fontWeight: 800 }}>On Going Projects</Text>
          </Box>
          {props.project.filter(
            (project) => project.status === Status.ONGOING || project.status === Status.PENDING
          ).length > 0 ? (
            <Grid _extra={{ className: "grid-cols-3 gap-4" }}>
              {props.project.reduce((acc: React.ReactNode[], project) => {
                if (project.status === Status.ONGOING || project.status === Status.PENDING) {
                  acc.push(
                    <GridItem
                      key={project.id}
